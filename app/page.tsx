      ...
      <section className="workspace">
        ...
        <div className="studio-grid">
          ...
          <aside className="right-column">
            <section className="card json-graph" aria-label="JSON graph">
              <div className="panel-heading compact"><div><h2>JSON graph</h2><p>Select a node to edit its properties or add children.</p></div></div>
              {nestingTargetPath && <span className="nesting-target">Target: {nodeAtPath(parsed.document as JsonObject, nestingTargetPath)?.type}</span>}
              <div className="json-graph-tree">{parsed.document ? outline(parsed.document) : <p className="empty-state">Fix the JSON to view its graph.</p>}</div>
            </section>
